...
* Simulate packet loss, delay, etc. on any nodes according to any given parameters and models.
* Collect network and user-defined data in detail, from each and every node.
* Cross-platform and high-precision.

## Introduction

#### Concept

* Network: a topological graph consist of *node*s, reflecting a real-world network for *packets* to transfer and route through.
* Node: a physical or logical device in the *network* allowing *packet*s to enter and leave. A *node* usually *connects*
  to other *node*s.
* Packet: simulated data packets transferring between *node*s, carrying the actual packet data with additional low-level protocol stack or simulator
  data.
...
The network is built by *node*s and *edge*s. Normally an *edge* connects only two *node*s, each on one end. In some special
cases, a *node* may *connect* to multiple (or none) incoming and/or outgoing *node*s.
...
After all necessary *node*s created, *connect* them with code to build the network. To do so, just set the *next node* correctly
for each *node* until all *edge*s are defined as expected.

In addition, ns-x provides a *builder* to facilitate the process. Instead of connecting *edge*s，it builds the network by
connecting all *path*s in one line of code.

A *path*, aka a *chain*, like the *path* concept in graph theory, represents a route along the *edge*s of a
graph.
